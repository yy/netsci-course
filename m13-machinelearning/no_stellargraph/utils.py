# THIS IS A COPY-PASTE (with minor modification) OF 
# https://stellargraph.readthedocs.io/en/latest/_modules/stellargraph/data/edge_splitter.html

# THIS WAS DONE TO ELIMINATE THE ASSIGNMENT'S DEPENDENCY ON STELLARGRAPH SINCE 
# IT IS CURRENTLY NOT SUPPORTED  ON M1/2 MAC'S AND GOOGLE COLLAB.



# THE FOLLOWING (INCLUDING THE LICENSE) IS COPIED.
...
class EdgeSplitter(object):
    ...
    def __init__(self, g, g_master=None):
        ...
        if not isinstance(g, nx.Graph):
            raise Exception("Input should be nx.Graph")

        if g_master is not None:
            if not isinstance(g_master, nx.Graph):
                raise Exception("Input should be nx.Graph")
    ...
    def train_test_split(
        ...
        Returns:
            ...
            :class: a NetworkX graph if the input
            graph was one.
        """
        if p <= 0 or p >= 1:
